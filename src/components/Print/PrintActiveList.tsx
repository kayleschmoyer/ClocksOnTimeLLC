...
interface PrintActiveListProps {
  records: WaitingListRecord[]
}

export const PrintActiveList = React.forwardRef<HTMLDivElement, PrintActiveListProps>(
  ({ records }, ref) => {
    return (
      <div ref={ref} style={styles.container}>
        <div style={styles.header}>
          <h1 style={styles.title}>Clocks on Time LLC</h1>
          <h2 style={styles.subtitle}>Active Waiting List</h2>
          <p style={styles.meta}>
            Printed: {new Date().toLocaleDateString()} &nbsp;|&nbsp; {records.length} Active Record{records.length !== 1 ? 's' : ''}
          </p>
        </div>
        ...
        <table style={styles.table}>
          <thead>
            <tr>
              {['#', 'Last Name', 'First Name', 'Date Entered', 'Phone', 'Date Called', 'Clock Type', 'Issue'].map(h => (
          ...
          <tbody>
            {records.map((r, i) => (
              <tr key={r.id} style={i % 2 === 0 ? styles.rowEven : styles.rowOdd}>
                ...
                <td style={{ ...styles.td, maxWidth: 180 }}>{r.issue}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
PrintActiveList.displayName = 'PrintActiveList'
